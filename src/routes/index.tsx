    ...
    >
      <header className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-primary">{t("liveModel")}</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="panel px-4 py-3 font-mono text-[11px] text-muted-foreground">
            <div>{t("session")} · <span className="text-foreground">poultry-arid-01</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <div className="lg:col-span-4 space-y-4">
          <ParameterSlider icon={Thermometer} label={t("airTemp")} unit="°C" value={T} min={-15} max={60} step={0.5} onChange={setT} accent="danger" hint={t("hintT")} />
          <ParameterSlider icon={Droplets} label={t("rh")} unit="%" value={RH} min={0} max={100} step={1} onChange={setRH} accent="primary" hint={t("hintRH")} />
          <ParameterSlider icon={FlaskConical} label={t("litterPH")} unit="" value={pH} min={1} max={14} step={0.1} onChange={setPH} accent="accent" hint={t("hintPH")} />
        </div>
        ...
        <div className="lg:col-span-8 space-y-5">
            ...
            <div className="panel p-5 flex flex-col">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{t("sensitivity")}</div>
                    <div className="text-sm font-medium">{t("nh3VsVar")}</div>
                  </div>
                </div>
                <div className="flex rounded-md border border-border overflow-hidden font-mono text-[11px]" dir="ltr">
              ...
              <div className="mt-2 flex flex-wrap gap-3 font-mono text-[10px] text-muted-foreground">
                <span className="flex items-center gap-1.5"><span className="inline-block w-3 h-px bg-[color:var(--color-safe)]" /> {t("safeCeiling")}</span>
                <span className="flex items-center gap-1.5"><span className="inline-block w-3 h-px bg-[color:var(--color-danger)]" /> {t("dangerLine")}</span>
                <span className="flex items-center gap-1.5"><span className="inline-block w-3 h-px bg-[color:var(--color-accent)]" /> {t("currentVal")}</span>
              </div>
            </div>
          </div>
          ...
          <div
            ...
            style={{ borderColor: out.factors.aridSynergy > 1.05 ? "var(--color-danger)" : "var(--color-border)" }}
          >
            ...
            >
              {out.factors.aridSynergy > 1.05 ? t("active") : t("inactive")} · ×{out.factors.aridSynergy.toFixed(2)}
            </div>
          </div>
        </div>
      </div>
